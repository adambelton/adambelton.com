# Implementation Overview: ThoughtForm
...
- ThoughtForm is the source of truth for its own product contracts and domain types.
- ThoughtForm product code uses `shared`, `server`, and `client` boundaries under `packages/products/src/thoughtform/`.
...
ThoughtForm is the first product inside that site, not the entire app.
...
/products/thoughtform               Product overview/demo page
/products/thoughtform/editor        ThoughtForm editor
...
personal-site/
  ...
  tasks/
    ...
    004-thoughtform-service-stub.md
...
- writing post types
- ThoughtForm conversation state types
- product IDs
...
```ts
export type ProductId = "thoughtform";
...
### ThoughtForm conversation types
...
export type ThoughtFormConversationResponse = {
...
Published writing should be a site-level concept, not a ThoughtForm-only concept.

ThoughtForm creates owner-scoped conversations that may produce private drafts.

Published drafts become website writing.

The website should not care whether a published piece started in ThoughtForm, was written manually, or came from another future product.

Suggested boundary:

```txt
Conversation process → ThoughtFormConversation
Private writing artifact → ThoughtFormDraft
Public website writing → WritingPost
```

When publishing from ThoughtForm, create or update a `WritingPost`.
...
GET  /products/thoughtform/session
POST /products/thoughtform/conversation/respond
GET  /products/thoughtform/conversations
GET  /products/thoughtform/conversations/:id
GET  /products/thoughtform/drafts/:id
PATCH /products/thoughtform/drafts/:id
POST /products/thoughtform/drafts/:id/publish
...
class ConversationService {
  constructor(params: {
    user: User;
    accessLevel: "owner" | "demo";
    conversation?: ThoughtFormConversation;
    message: string;
  ...
  async call(): Promise<ThoughtFormConversationResponse> {
    // Build system prompt
    // Build messages
    // Call AI provider
...
3. Creates `ThoughtFormConversation` if `conversationId` is null.
...
11. If composition is accepted, creates or updates the associated `ThoughtFormDraft`.
...
When publishing from ThoughtForm:
...
ThoughtForm conversation policy belongs in:
...
- ThoughtForm conversation types exist
...
- explicit ThoughtForm route and service
