...
Replace the deterministic ThoughtForm stub response path with a minimal real LLM-backed response flow while preserving the product/host dependency boundary.
...
- Add a ThoughtForm-owned conversation model port.
- Inject the host AI adapter into the ThoughtForm conversation service from `apps/api`.
