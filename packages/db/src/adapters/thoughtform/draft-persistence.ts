...
export function createPrismaDraftPersistence(
  ...
  async function load(conversationId: string): Promise<DraftingState | null> {
    const conversation = await prisma.thoughtFormConversation.findFirst({
    ...
    if (!conversation.draft) return structuredClone(EMPTY_DRAFTING_STATE);
  ...
  return {
    async initialize(conversationId) {
      const exists = await prisma.thoughtFormConversation.findFirst({
        where: { id: conversationId, userId },
        select: { id: true },
      });
    ...
    async loadCompletedOperation(conversationId, operationId) {
      const operation = await prisma.thoughtFormOperation.findUnique({
        where: {
    ...
    async commit(input: DraftCommitInput) {
      ...
      const duplicate = await prisma.thoughtFormOperation.findUnique({
      ...
      try {
        ...
        if (!current.draft && next.draft) {
          await transaction.thoughtFormDraft.create({ data: {
          ...
          const updated = await transaction.thoughtFormDraft.updateMany({
        ...
        for (const revision of next.revisions.slice(current.revisions.length)) {
          await transaction.thoughtFormDraftRevision.create({ data: {
        ...
        if (
          ...
          await transaction.thoughtFormRevisionProposal.create({ data: {
          ...
          const proposalUpdated = await transaction.thoughtFormRevisionProposal.updateMany({
            where: {
          ...
          for (const version of nextProposal.versions.slice(currentProposal.versions.length)) {
            await transaction.thoughtFormRevisionProposalVersion.create({ data: { proposalId: nextProposal.id, revision: version.revision, proposedContent: version.proposedContent, intendedEffect: version.intendedEffect, createdAt: new Date(version.createdAt) } });
          }
        }
        ...
        if (isUniqueConstraintError(error)) {
          const completed = await prisma.thoughtFormOperation.findUnique({
            where: { conversationId_operationId: { conversationId: input.conversationId, operationId: input.operationId } },
            select: { id: true },
          });
          if (completed) {
    ...
    async delete(conversationId) {
      await prisma.thoughtFormDraft.deleteMany({ where: { conversationId, conversation: { userId } } });
    },
  };
}
