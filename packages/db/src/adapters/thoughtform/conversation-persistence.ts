...
export function createPrismaConversationPersistence(
  ...
  async function load(conversationId: string) {
    const row = await prisma.thoughtFormConversation.findFirst({
  ...
  return {
    async initialize(input) {
      const row = await prisma.thoughtFormConversation.create({
        data: {
    ...
    async list() {
      const rows = await prisma.thoughtFormConversation.findMany({
        where: { userId },
    ...
    async commit(input: ConversationCommitInput) {
      ...
      try {
        ...
        const row = await transaction.thoughtFormConversation.findFirst({
          where: { id: input.conversationId, userId },
          select: { nextMessagePosition: true },
        });
        if (!row) throw new ConversationCommitConflictError();
        const update = await transaction.thoughtFormConversation.updateMany({
          where: {
            id: input.conversationId,
            userId,
        ...
        if (addedMessages.length > 0) {
          await transaction.thoughtFormConversationMessage.createMany({
...
type SelectedConversationRow = Prisma.ThoughtFormConversationGetPayload<{
