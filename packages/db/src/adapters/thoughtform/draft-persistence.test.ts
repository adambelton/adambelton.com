...
import { createPrismaDraftPersistence } from "packages/db/src/adapters/thoughtform/draft-persistence";
import { createDraftStore } from "packages/products/src/thoughtform/server/capabilities/drafting";

describe("Prisma ThoughtForm draft persistence", () => {
...
function createFakeDraftDatabase(conversations: ConversationRow[]) {
  ...
  const transaction = {
    ...
    thoughtFormRevisionProposalVersion: {
  ...
  const client = {
    thoughtFormConversation: { findFirst: selectedConversation },
    thoughtFormOperation: {
