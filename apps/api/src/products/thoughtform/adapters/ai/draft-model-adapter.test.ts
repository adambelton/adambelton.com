...
describe("draft model adapters", () => {
  it("maps composition requests and parses structured provider output", async () => {
    ...
    expect(requests[0]?.system).toContain("minimum coherent private articulation");
    expect(requests[0]?.system).toContain("one sentence, a paragraph, a list, or a longer account");
